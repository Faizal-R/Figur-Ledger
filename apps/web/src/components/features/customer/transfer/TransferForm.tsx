...
import { CheckCircle2, ShieldCheck, ArrowRight, Search, AlertCircle, IndianRupee } from "lucide-react";
...
export default function TransferForm() {
  ...
  const handleTransfer = () => {
    if (!fromAccountId) { setError("Please select an account to send from"); return; }
    if (!verifiedAccount) { setError("Please verify the recipient first"); return; }
    const numericAmount = Number(amount);
    if (!numericAmount || numericAmount <= 0) { setError("Please enter a valid amount"); return; }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        <label className={cn("text-[10px] font-bold uppercase tracking-widest opacity-60", t.text.muted)}>Select Source</label>
        <AccountSelector onSelect={setFromAccountId} className="no-scrollbar" />
      </div>

      {/* 2. Recipient */}
      <div className="space-y-3">
        <label className={cn("text-[10px] font-bold uppercase tracking-widest opacity-60", t.text.muted)}>Recipient Account ID</label>
        <div className="relative">
           ...
           <div className="absolute right-1.5 top-1/2 -translate-y-1/2">
             <button
               ...
               disabled={verifyAccountMutation.isPending || !receiverAccountInput}
               className={cn(
      ...
      {/* 3. Amount */}
      <div className="space-y-3">
        <label className={cn("text-[10px] font-bold uppercase tracking-widest opacity-60", t.text.muted)}>Amount to Send</label>
        <div className="relative">
      ...
      <div className="pt-2 space-y-4">
        ...
        <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-slate-50 dark:bg-white/5">
           <p className={cn("text-[10px] font-bold opacity-50", t.text.muted)}>Network Fee</p>

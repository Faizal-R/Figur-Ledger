...
const RecentPaymentsView: React.FC<RecentPaymentsViewProps> = ({
  payments,
  currentPage,
  totalPages,
  onPageChange,
    ...
    return (
      ...
      >
        <div className="w-24 h-24 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 flex items-center justify-center mb-8 relative">
          ...
          <History
            size={40}
            className={`${t.text.muted} opacity-40 relative z-10`}
          />
        </div>
        <div className="text-center space-y-3 max-w-sm">
          <h3
            className={`text-2xl font-black tracking-tight ${t.text.heading}`}
          >
            System Idle
          </h3>
          <p className={`text-sm font-medium ${t.text.body} opacity-50`}>
            No transmission logs detected in the recent activity matrix. Proceed
            with a transaction to initialize logs.
          </p>
        </div>
      </motion.div>
    );
            ...
            >
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                ...
                <div className="flex items-center gap-5">
                  <div
                    className={`w-16 h-16 rounded-2xl ${t.card.base} border border-black/5 dark:border-white/5 flex items-center justify-center shadow-lg group-hover:rotate-6 transition-transform`}
                  >
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      <h4
                        className={`${t.text.heading} font-black text-xl tracking-tight`}
                      >
                        {payee?.name || "Service Node"}
                      </h4>
                      <span
                        className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${status.bg} ${status.color} border ${status.border}`}
                      >
                        {status.text}
                      </span>
                    </div>
                    <p
                      className={`${t.text.muted} text-[10px] font-black uppercase tracking-widest opacity-60`}
                    >
                ...
                <div className="flex items-center justify-between md:justify-end gap-12 flex-1">
                  <div className="text-left md:text-right space-y-1">
                    <p
                      className={`text-[9px] font-black uppercase tracking-[0.2em] ${t.text.muted} opacity-40`}
                    >
                      Total Allocated
                    </p>
                    <div className="flex items-baseline gap-2 md:justify-end">
                      <span
                        className={`text-2xl font-black tracking-tighter ${t.text.display}`}
                      >
                        ₹{payment.amount}
                      </span>
                      <span
                        className={`text-[10px] font-black ${t.text.muted}`}
                      >
                        INR
                      </span>
                    </div>
